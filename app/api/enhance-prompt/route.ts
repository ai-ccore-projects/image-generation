...
import { chatWithOllama, getTextProviderConfig } from '@/lib/llm-providers'
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Check if the selected API provider is configured
    if (textProvider.provider === 'openai' && (!process.env.OPENAI_API_KEY || process.env.OPENAI_API_KEY === 'dummy-key-for-build')) {
      return NextResponse.json({
        error: 'API Configuration Error',
        message: 'OpenAI API key is not configured. Set OPENAI_API_KEY, or set AI_TEXT_PROVIDER=ollama and configure OLLAMA_MODEL.',
...
`
    ...
    let contentModerated = false

    // Step 1: Content Moderation - Check for inappropriate content when OpenAI is configured.
    if (contentToModerate.trim() && process.env.OPENAI_API_KEY && process.env.OPENAI_API_KEY !== 'dummy-key-for-build') {
    ...
    // Step 2: Enhance the prompt with OpenAI or a local Ollama LLM.
    const enhancedPrompt = textProvider.provider === 'ollama'
    ...
    if (error instanceof Error) {
      return NextResponse.json({
        ...
        message: error.message.includes('API key') 
          ? 'AI provider configuration error. Please check your OpenAI or Ollama settings.'
