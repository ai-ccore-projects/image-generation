...
import { chatWithOllamaVision, getVisionProviderConfig } from "@/lib/llm-providers"
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (visionProvider.provider === "openai" && !process.env.OPENAI_API_KEY) {
      return NextResponse.json({
        error: "OpenAI API key is not configured. Set OPENAI_API_KEY, or set AI_VISION_PROVIDER=ollama and configure OLLAMA_VISION_MODEL.",
      }, { status: 503 })
    }

    const systemPrompt = `You are an expert art critic and AI image generation evaluator. Your task is to compare a reference image with an AI-generated recreation and provide detailed scoring and feedback.
...
Be constructive and encouraging while being honest about areas that need work.`
    ...
    const userMessage = `Please compare these two images:
...
The first image is the reference that should be recreated. The second image is the AI-generated recreation. Please evaluate how well the recreation matches the reference image.`
    ...
    const content = visionProvider.provider === "ollama"
      ? await chatWithOllamaVision({
          systemPrompt,
    ...
    if (!content) {
      throw new Error(`No response from ${visionProvider.model}`)
    }

    console.log(`${visionProvider.model} Raw Response:`, content)
    ...
    try {
      ...
      return NextResponse.json({ 
        error: `Failed to parse ${visionProvider.model} response as JSON`,
