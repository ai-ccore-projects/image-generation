-- Community gallery RPC functions (from scripts/05-create-gallery-functions.sql)
...
BEGIN
  ...
  FROM image_likes
  WHERE image_likes.image_id = toggle_image_like.image_id
    AND image_likes.user_id = toggle_image_like.user_id;
  ...
  IF existing_like_id IS NOT NULL THEN
    ...
    WHERE id = toggle_image_like.image_id
...
BEGIN
  ...
  VALUES (record_image_view.image_id, record_image_view.user_id, NOW())
  ON CONFLICT (image_id, user_id) DO UPDATE SET viewed_at = NOW();

  -- Update view count
  UPDATE generated_images
  SET views_count = COALESCE(views_count, 0) + 1
  WHERE id = record_image_view.image_id;
